...
Lending queries the configured EVM client's `getCode` method when `chain.id` identifies Ethereum mainnet. Without both fields, the SDK queries the Liquidium SDK API. RPC and API failures do not block the outflow.
