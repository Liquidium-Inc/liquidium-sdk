...
Malformed and reserved Ethereum addresses throw `LiquidiumError` with `LiquidiumErrorCode.INVALID_ADDRESS`. Asset, chain, or destination-family mismatches throw `LiquidiumErrorCode.VALIDATION_ERROR`. Ethereum lending destinations with deployed bytecode throw `LiquidiumErrorCode.CONTRACT_DESTINATION_UNSUPPORTED`; Simple Loan creation reports backend destination rejection through its SDK API error. Catch these at the form boundary before retrying the flow.

Ethereum lending outflow validation uses a configured EVM client with `getCode`, or the Liquidium SDK API when no bytecode-capable client is configured. Validation errors block the outflow. Treat `SERVICE_UNAVAILABLE`, `NETWORK_ERROR`, and `REQUEST_TIMEOUT` as retriable SDK API validation failures; do not proceed until a check succeeds.
