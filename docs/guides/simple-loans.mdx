...
`loan.position.borrowedAmount`, `loan.position.totalDebtAmount`, and
`loan.repayment.debtAmount` contain current debt, including accrued interest.
`loan.position.debtInterestAmount` is a reporting value that can include interest
already paid. Do not add it to current debt. The same rule applies to
`Position.borrowed` and `Position.debtInterest`.
