...
| Validate amounts | `client.quote.calculateLtv(...)` | Block too-small borrow amounts or an invalid loan-to-value (LTV) ratio before creation |
...
`client.simpleLoans.create(...)` rejects a frozen collateral or borrow pool with `LiquidiumErrorCode.POOL_FROZEN` before it sends the API request.
