...
String `find(...)` calls the SDK API search index and returns hydrated loans with activities. It supports short references, numeric canister loan id strings, generated deposit or repayment addresses, borrow or refund destination addresses, and indexed transaction ids. Pass `{ ref }`, `{ loanId }`, or a `bigint` when you already have an exact identifier and want direct canister lookup.

`findByAddress(...)` is deprecated and remains available for legacy integrations that need lightweight candidate records.
...
| `loan.status` | Lifecycle state, including `awaiting_deposit`, `deposit_detected`, `active`, `settling`, `closed`, or `expired` |
...
`client.instantLoans.find(...)` wraps these fields in `result.loan` and includes active and completed flow data in `result.activities`.
