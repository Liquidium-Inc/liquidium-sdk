...
| Repay loan | Read `loan.repayment` | If `loan.repayment.amount > 0n`, ask the user to send it to `loan.repayment.target` |
...
| `loan.repayment` | Repayment quote. Amount fields are zero when no debt exists. |
| `loan.repayment.amount` | Fee-inclusive repayment amount |
| `loan.repayment.target` | Address or account where the user sends repayment |
