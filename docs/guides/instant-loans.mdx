...
Use `find(...)` when the user has a short reference, numeric loan id, address, or transaction id:

```ts
const results = await client.instantLoans.find(addressOrTxidOrRef);
```

`find(...)` returns hydrated loans and activities. It supports short references, numeric canister loan ids, generated deposit or repayment addresses, borrow or refund destination addresses, and indexed transaction ids.
