...
| `agent` | Use a preconfigured ICP `Agent`. When provided, the SDK ignores `icHost` and `identity`. |
...
| `evmRpcUrl` | Create a viem public client for ERC-20 reads and native ETH destination checks. |
| `evmRpcHeaders` | Add headers to EVM RPC requests. |
| `evmPublicClient` | Use an existing viem-compatible client. `readContract` is required; mainnet `chain` and `getCode` enable native ETH destination checks. The SDK uses this client instead of `evmRpcUrl` and `evmRpcHeaders`. |

Pass a custom agent when your app already owns ICP transport and identity setup:
