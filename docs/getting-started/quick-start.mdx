---
title: Create and restore a Simple Loan
...
- Install `@liquidium/client`.
- Prepare an Ethereum address to receive the borrowed USDC.
- Prepare a Bitcoin address to receive a collateral refund.
- Confirm that your runtime provides `fetch` and `BigInt`.
...
see [Handle SDK errors](../guides/error-handling.mdx#handle-sdk-errors).
