...
`confirmations` and `requiredConfirmations` are populated only while `state` is
`confirming`. Confirmation progress never exceeds the required count. Both
fields are `null` after the operation moves to `processing` because any remaining
work belongs to Liquidium's internal settlement lifecycle.
