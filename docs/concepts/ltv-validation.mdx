...
`borrowAmount` is the amount the borrower receives. If the borrow pool charges an activation fee, the SDK calculates `fee = borrowAmount * pool.activationFee / 10_000n` (rounded down in base units) and uses `borrowAmount + fee` as the opening debt for LTV and required-collateral quotes. The `borrowUsd` quote field is the value of this opening debt, while the returned `borrowAmount` stays the requested amount. Read `activationFee` from `client.market.listPools()`; it is `0n` for pools on older canisters. The pool fee can change between a quote and a borrow, so recheck the pool before submission.
