...
The SDK also validates minimum borrow amounts. BTC borrows must be at least `5_100n` sats. ETH borrows must be at least `5_000_000_000_000_000n` wei (`0.005 ETH`). USDC and USDT borrows must be at least `1_000_000n` base units. Use `getMinimumBorrowAmount(asset)` to display the same minimum before the user submits.
