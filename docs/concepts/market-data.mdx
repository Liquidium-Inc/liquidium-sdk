...
`activationFee` is the borrow pool's activation fee in basis points (`50n` means 0.5%). It is `0n` if the canister has no activation-fee field. Borrowers receive the requested amount but owe that amount plus the fee, rounded down in base units. The canister can reject a borrow if its fee-inclusive debt exceeds available liquidity, the borrow cap, or the LTV limit.
