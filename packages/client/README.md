...
| --- | --- |
| [`examples/instant-loans-flow`](https://github.com/Liquidium-Inc/liquidium-sdk/tree/main/examples/instant-loans-flow) | Full instant loan UX with Dynamic wallet connection, pool selection, LTV preview, loan creation, status reload, activity status, and address recovery |
| [`examples/vite-react-dynamic`](https://github.com/Liquidium-Inc/liquidium-sdk/tree/main/examples/vite-react-dynamic) | Developer tool for calling SDK methods directly, including instant loan method templates |
...
This README is the npm-facing overview for `@liquidium/client`. Most new integrations should start with `client.instantLoans`; profile-based deposit-address flows and ETH contract-interaction flows are advanced paths.
